...
unsigned char rxBuffer[256];							// приемный буффер  TWI
unsigned char txBuffer[64];								// передающий буффер TWI
unsigned char rxBufferUART[64];					// накапливающий приемный буффер UART
...
//}
  		while (txPack);			// ждем пока вычитается ответ и
//void reboot (void)
//{		
//    	delay_ms(25);
...
const char _PT_GETSTATE_[]={20,2,0,'P','o','r','t',' ','R','S','-','2','3','2',' ',' ',' ',' ',100,255};
static void GetState(void)
{
	register unsigned char a=0, crc=0;

		if (!(rxBuffer[4]))	
			{
				memcpyf(txBuffer, _PT_GETSTATE_, _PT_GETSTATE_[0]+1); // 0 пакет
			}
		else                                                                                                             // 1 пакет
			{
				txBuffer[a++] = 5;				 			// длина пакета
				txBuffer[a++] = 0;
				txBuffer[a++] = pAddr;
				txBuffer[a++] = lAddr;
				txBuffer[a++] = 255;
...
{
	register unsigned char i,a=0,crc=0;                    
...
//print = 1;
		...
		else	if(rxBuffer[2] == TO_MON)					// обрабатываем пакет по адресу MONITOR
			{
				...
				{ 												///////////// иначе ретранслируем. Только при локальной адресации TWI
					if (!(GlobalAddr))
						{
							for (a = 0;a<= (rxBuffer[1]+1); a++) putchar (rxBuffer [a]);
        				}
					rxPack = 0;					// пакет обработан
				}
			}	
...
// Обработка пакета, принятого по UART
	void workUARTpack (void)
		{
			#asm("cli")
			memcpy(txBuffer, rxBufferUART, rxBufferUART[0]+1); // 0 пакет
			#asm("sei")
...
void _print (unsigned char data)
	{
	unsigned char a=0, crc =0 ;
				txBuffer[a++] = 3;				 			// длина пакета
				txBuffer[a++] = 0xaa;		  			// лог. адрес
				txBuffer[a++] = data;		
...
//		if ((!(dPresent)) || (tstPort)) readAddrDevice(); 	 	// если нет устройства - ищем его
		if (tstPort) readAddrDevice(); 	 	// если нет устройства - ищем его
		if (rxPack) workINpack();			// принят пакет TWI 
		if (rxPackUART) workUARTpack();			// обрабатываем принятый пакетUART
